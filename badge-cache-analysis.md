...
- 徽章 URL 的典型用法是 `<img src="/api/badge/1/status">` 或 Markdown 图片语法 `![status](...)`
- 代码中没有设置 `crossorigin` 属性相关的处理逻辑，因此不讨论带凭证的 CORS 场景
...
1. **跨域策略差异明显**：
   - 状态页面徽章（生产环境）：不设置 CORS 头，**可以被 `<img>` 嵌入展示**，但**无法被 JavaScript 脚本读取**（保护状态页数据不被爬取）
   - 单个监控器徽章：始终设置 `Access-Control-Allow-Origin: *`，**既可以被嵌入展示，也可以被脚本读取**（便于第三方集成）
